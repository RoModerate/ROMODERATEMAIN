...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Submit a report for a listing or seller
  app.post("/api/marketplace/reports", requireAuth, async (req: any, res) => {
    try {
      ...
      if (!['listing', 'seller'].includes(type)) {
        return res.status(400).json({ error: "Invalid report type" });
      }

      // Create the report (store in database when available)
      const report = {
      ...
      const adminChannelId = '1323785162043162646';
      try {
        const { Client, EmbedBuilder, GatewayIntentBits } = await import('discord.js');
        ...
        if (client) {
          ...
          if (channel && channel.isTextBased()) {
            const embed = new EmbedBuilder()
              .setTitle(`New ${type === 'listing' ? 'Listing' : 'Seller'} Report`)
              .setColor(0xFF6B6B)
              .addFields(
                { name: 'Report ID', value: report.id, inline: true },
                { name: 'Type', value: type, inline: true },
                { name: 'Target ID', value: targetId, inline: true },
                { name: 'Reporter', value: req.user.username, inline: true },
                { name: 'Reason', value: reason.substring(0, 1000) }
              )
              .setTimestamp();
        ...
        console.error('[Discord] Failed to send report notification:', discordError);
      }
  ...
  // Rate limiting for AI chat (per user, 10 requests per minute)
  const aiChatRateLimits = new Map<string, { count: number; resetAt: number }>();
  ...
  app.post("/api/ai/chat", requireAuth, async (req: any, res) => {
    try {
      ...
      // Check rate limit
      let userLimit = aiChatRateLimits.get(userId);
      if (!userLimit || now > userLimit.resetAt) {
        userLimit = { count: 0, resetAt: now + AI_RATE_WINDOW };
        aiChatRateLimits.set(userId, userLimit);
      }
      ...
      if (!HUGGINGFACE_API_KEY) {
        ...
        const fallbackResponses = [
          "I can help you find great deals on the marketplace! Try searching for specific items or browsing categories.",
          "Looking to sell? Make sure to add clear photos and detailed descriptions to attract buyers!",
          "Need help with a transaction? Our escrow system keeps both buyers and sellers protected.",
          "You can report any suspicious listings or users using the menu on each listing card.",
          "Check out trending items to see what's popular right now!",
        ];
      ...
      const response = await fetch(
        "https://api-inference.huggingface.co/models/microsoft/DialoGPT-medium",
        {
          ...
          body: JSON.stringify({
            inputs: message.substring(0, 500), // Enforce max length
            parameters: {
              max_length: 150,
              temperature: 0.7,
            }
      ...
      if (!response.ok) {
        // Fallback to marketplace-specific responses if API fails
        const fallbackResponses = [
          "I can help you find great deals on the marketplace! Try searching for specific items or browsing categories.",
          "Looking to sell? Make sure to add clear photos and detailed descriptions to attract buyers!",
          "Need help with a transaction? Our escrow system keeps both buyers and sellers protected.",
      ...
      const data = await response.json();
      let aiResponse = "I'm here to help with the marketplace!";
      ...
      res.json({ response: "I'm having trouble connecting right now. Try asking about buying, selling, or navigating the marketplace!" });
    }
  });

  // ===== ADMIN MARKETPLACE ROUTES =====

  // Get all listings for admin (includes all statuses)
  app.get("/api/admin/marketplace/listings", requireAdminAuth, async (req: any, res) => {
    try {
  ...
  app.delete("/api/admin/marketplace/listings/:id", requireAdminAuth, async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      try {
        ...
        if (client) {
          ...
          if (channel && channel.isTextBased()) {
            const embed = new EmbedBuilder()
              .setTitle('Listing Removed by Admin')
              .setColor(0xFF0000)
              .addFields(
                { name: 'Listing ID', value: id, inline: true },
                { name: 'Title', value: listing.title, inline: true },
                { name: 'Seller ID', value: listing.sellerId, inline: true },
                { name: 'Admin', value: req.admin?.username || 'Unknown', inline: true },
        ...
        console.error('[Discord] Failed to send admin action notification:', discordError);
      }
  ...
  app.patch("/api/admin/marketplace/listings/:id", requireAdminAuth, async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      const updated = await storage.updateListing(id, { status });
      ...
      try {
        ...
        if (client) {
          ...
          if (channel && channel.isTextBased()) {
            const embed = new EmbedBuilder()
              .setTitle('Listing Status Updated')
              .setColor(status === 'suspended' ? 0xFF6B6B : 0x00FF00)
              .addFields(
                { name: 'Listing ID', value: id, inline: true },
                { name: 'Title', value: listing.title, inline: true },
  ...
  app.get("/api/admin/marketplace/stats", requireAdminAuth, async (req: any, res) => {
    try {
      const listings = await storage.getListings({ limit: 10000 });
      
      const stats = {
        totalListings: listings.length,
        activeListings: listings.filter((l: any) => l.status === 'active').length,
        soldListings: listings.filter((l: any) => l.status === 'sold').length,
        suspendedListings: listings.filter((l: any) => l.status === 'suspended').length,
        totalValue: listings.reduce((sum: number, l: any) => sum + (l.price || 0), 0),
      };
